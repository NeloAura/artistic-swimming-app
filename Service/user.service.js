...
function allDetails(user) {
    const { id, email,firstname, lastname, svgSrc, mobile, role } = user;
    return { id, email,firstname, lastname, svgSrc, mobile, role };
}

async function sendVerificationEmail(user, origin) {
    let message;
    if (origin) {
        const verifyUrl = `http://localhost:3000/users/verify-email?token=${user.verificationToken}`;
        message = `<p>Please click the below link to verify your email address:</p>
                   <p>Press<a href=${verifyUrl}>Click here</a> to proceed</p>`;
    } else {
